//编译命令是 arm-linux-gcc projectone.c -o projectone -I /usr/local/arm/include/ -L /usr/local/arm/lib -lts
#include <stdio.h>
...
#define START "/mnt/udisk/demo/start.bmp"
#define BLANK "/mnt/udisk/demo/blank.bmp"
#define ERROR "/mnt/udisk/demo/error.bmp"
#define MAIN "/mnt/udisk/demo/main.bmp"
#define MUSIC "/mnt/udisk/demo/musicbrowse.bmp"
#define PICTURE "/mnt/udisk/demo/picborwse.bmp"
#define SUCCESS "/mnt/udisk/demo/success.bmp"
#define LOGINUI "/mnt/udisk/demo/loginUI.bmp"
#define FB_PATH "/dev/fb0"
#define MUSICPATH "/mnt/udisk/demo/entertainment/"//指定某个目录，里面存放MP3，BMP格式的文件
...
int mainUI(struct tsdev *ts)//通过返回值1通知
{
     while(1)
         ...
         show_pic(MAIN,0,0);//加载主界面图片
     	   ts_read(ts, &samp, 1);
     	   printf("11111%s",music_head->prev->strpath);
...
void picborwse(struct ts_sample samp,struct tsdev *ts,struct path*bmp_head)
{
                   ...
                   while(1)
                             {
                               ...
                               show_pic(p->strpath,0,0);
                                //制做一个时间段，每0.01秒读屏幕一次，压力大于零的跳出循环
                                 for(i=0;i<100;i++)
                                      {
                                        
                                        usleep(10000);//参数单位是百万分之一秒
  ...
  /*else if(is_scope(samp,640,104,761,162))
      //手动浏览
          {
            //做一个滑动的
          }*/
